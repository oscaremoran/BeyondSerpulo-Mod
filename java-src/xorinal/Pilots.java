...
public class Pilots {
    ...
    private static void snapshotDeployed() {
        try {
            ...
            Log.info("[Xorinal] snapshotDeployed: " + entries.size + " entries for " + sk);
        } catch (Exception ex) { Log.err("[Xorinal] snapshotDeployed: " + ex); }
    }
    ...
    private static void parkInSector() {
        try {
            ...
            Log.info("[Xorinal] parkInSector: sector=" + sk + " byUnit=" + byUnit.size);
            ...
            Log.info("[Xorinal] parkInSector: wrote " + entries.size + " entries for " + sk);
            byUnit.clear();
            nextBarkAt.clear();
            lastUnitPos.clear();
            lastUnitType.clear();
            ...
            downed.clear();
            saveDeployed();
        } catch (Exception ex) { Log.err("[Xorinal] parkInSector: " + ex); }
    }

    private static void reattachOnLoad() {
        try {
            String sk = currentSectorKey();
            if (sk == null) { Log.info("[Xorinal] reattach: no sector key"); return; }
            lastSectorKey = sk;
            Seq<DeployedEntry> bucket = deployedBySector.get(sk);
            if (bucket == null || bucket.isEmpty()) { Log.info("[Xorinal] reattach: no parked pilots for " + sk); return; }
            ...
            Log.info("[Xorinal] reattach: " + bucket.size + " parked pilots for " + sk
            ...
            for (DeployedEntry de : bucket) {
                ...
                Log.info("[Xorinal] reattach: candidates of type=" + de.unitTypeName + " count=" + matchCount
                    + " savedAt=(" + de.x + "," + de.y + ")");
                ...
                if (pick != null) {
                    ...
                    Log.info("[Xorinal] reattach: " + (de.pilot.name == null ? "?" : de.pilot.name)
                        + " -> unit " + pick.id + " (" + pick.type.name + ") dist=" + (float)Math.sqrt(anyDist));
                } else {
                    Log.info("[Xorinal] reattach: no matching unit for "
        ...
        } catch (Exception ex) { Log.err("[Xorinal] reattachOnLoad: " + ex); }
    }
    ...
    private static void saveDeployed() {
        ...
        } catch (Exception ex) { Log.err("[Xorinal] saveDeployed: " + ex); }
    }
    ...
    private static void loadDeployed() {
        ...
        } catch (Exception ex) { Log.err("[Xorinal] loadDeployed: " + ex); }
    }
    ...
    public static boolean parkPilotForLaunch(PilotData pilot, Unit unit, String destSectorKey) {
        try {
            ...
            saveDeployed();
            return true;
        } catch (Exception ex) { Log.err("[Xorinal] parkPilotForLaunch: " + ex); return false; }
    }
    ...
    public static void save() {
        try {
            ...
            Log.err("[Xorinal] Pilots.save: " + ex);
        }
    }
    ...
    public static void load() {
        try {
            ...
            Log.err("[Xorinal] Pilots.load: " + ex);
        }
    }
}
