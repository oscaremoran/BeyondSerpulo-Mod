...
public class Bloomheart {
    ...
    private static void tick() {
        ...
        if (!armed) {
            ...
            Log.info("[BeyondSerpulo] Bloomheart encounter armed (HP " + maxBossHp + ")");
        }
        ...
        if (phase == 1 && boss.health <= maxBossHp * PHASE2_HP_FRACTION) {
            phase = 2;
            pulseTimer = 0f;
            Log.info("[BeyondSerpulo] Bloomheart phase 2 (HP " + boss.health + "/" + maxBossHp + ")");
            announceOpens();
            pulse(boss);
        }
    ...
    private static Unit spawnAtEnemyCore() {
        try {
            ...
            Log.info("[BeyondSerpulo] Bloomheart auto-spawned at enemy core");
            return u;
        } catch (Exception ex) {
            Log.err("[BeyondSerpulo] Bloomheart spawn boss: " + ex);
            return null;
        }
    }
    ...
    private static void spawnUnit(UnitType type, Team team, float ax, float ay) {
        try {
            ...
            Log.err("[BeyondSerpulo] Bloomheart spawn: " + ex);
        }
    }
    ...
    private static void pulse(Unit anchor) {
        try {
            ...
            } catch (Exception ex) { Log.err("[BeyondSerpulo] pulse cloud: " + ex); }
            Log.info("[BeyondSerpulo] Bloomheart pulse fired");
        } catch (Exception ex) {
            Log.err("[BeyondSerpulo] Bloomheart pulse: " + ex);
        }
    }
