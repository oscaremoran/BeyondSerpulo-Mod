...
public class Difficulties {
    ...
    public static void init() {
        try {
            ...
            Log.err("[BeyondSerpulo] Difficulties: failed to acquire Unsafe: " + e);
            return;
        }
        ...
        try {
            ...
            if (vanilla[vanilla.length - 1].name().equals("annihilation")) {
                Log.info("[BeyondSerpulo] Difficulties already injected, skipping");
                return;
            }
            ...
            Log.info("[BeyondSerpulo] Injected " + extras.length + " difficulties; total now = " + Difficulty.values().length);
        } catch (Exception e) {
            Log.err("[BeyondSerpulo] Difficulties.init failed: " + e);
        }
    }
